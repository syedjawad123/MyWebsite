...
public class SapaadTrailPage extends Config {
	...
	public void enter_form_details(String name, String restaurantname, String phone) {
		...
		wait.until(ExpectedConditions.elementToBeClickable(txt_restaurantname)).sendKeys(restaurantname);
		wait.until(ExpectedConditions.elementToBeClickable(txt_phone)).sendKeys(phone);
		if (i_agree_checkbox.isSelected()) {
			System.out.println("thank you for checking the box");
		} else {
			wait.until(ExpectedConditions.elementToBeClickable(i_agree_checkbox)).click();
		}
	}
